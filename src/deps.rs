...
pub async fn ensure_ytdlp() -> Result<()> {
    ...
    #[cfg(windows)]
    {
        ...
        if check_command("pip").await || check_command("pip3").await {
            let pip_cmd = if check_command("pip3").await {
                "pip3"
            } else {
                "pip"
            };
...
async fn download_ytdlp_from_github() -> Result<()> {
    ...
    let download_url = ytdlp_asset["browser_download_url"]
        .as_str()
            ...
            .await
        {
            if output.status.success() {
                ...
                if current_version == tag_name.trim_start_matches('v') {
                    println!(
                        "✓ yt-dlp is already up to date (version {})",
                        current_version
                    );
                    return Ok(());
                }
...
async fn download_mpv_from_github() -> Result<()> {
    ...
    let download_url = mpv_asset["browser_download_url"]
        .as_str()
        .ok_or_else(|| anyhow::anyhow!("Download URL not found"))?;
    let filename = mpv_asset["name"]
        .as_str()
...
async fn check_local_mpv() -> bool {
    ...
    if mpv_exe.exists() {
        // Test if it works
        if let Ok(output) = TokioCommand::new(&mpv_exe).arg("--version").output().await {
            return output.status.success();
        }
    }
...
async fn check_command(cmd: &str) -> bool {
    ...
    if let Ok(output) = TokioCommand::new(cmd).arg("--version").output().await {
        if output.status.success() {
            return true;
        }
    }
    ...
    if let Ok(output) = TokioCommand::new(cmd).arg("-v").output().await {
        if output.status.success() {
            return true;
        }
    }
    ...
    if let Ok(output) = TokioCommand::new(cmd).arg("--help").output().await {
        if output.status.success() {
            return true;
        }
    }
...
async fn should_check_for_updates(tool: &str) -> bool {
    ...
    let check_file = app_data
        .join("rustyoutube")
        .join(format!("{}_last_check.txt", tool));
...
async fn upgrade_mpv_from_github() -> Result<()> {
    ...
    if let Ok(output) = TokioCommand::new(&mpv_path).arg("--version").output().await {
        if output.status.success() {
            ...
            println!(
                "Current mpv version: {}, Latest release: {}",
